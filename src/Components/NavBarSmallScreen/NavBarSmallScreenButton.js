...
function NavBarSmallScreenButton(props) {
  ...
  return (
    <button
      className="navbar-small-button text-4xl fixed bottom-10 p-3 z-[9] left-1/2  -translate-x-1/2 bg-gradient-to-b from-primary to-secondary text-white rounded-full hover:scale-105 transition-all duration-300 active:scale-95 shadow-xl"
      onClick={props.navStateSmallChanger}
    >
      {props.state ? <MdClose /> : <RiMenu5Fill />}
    </button>
  );
}
