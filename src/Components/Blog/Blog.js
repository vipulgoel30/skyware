      ...
      <div className="flex w-screen justify-center">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 p-10 gap-x-10 gap-y-20 max-w-7xl">
          {blogs.map(({ title, desc, image, url, date, author, tags }, i) => (
            <div
              key={i}
              className="flex flex-col  rounded-xl shadow-xl w-full gap-6 border-[1.5px] hover:border-black"
            >
              ...
              <ul className="gap-x-3 gap-y-2 flex flex-wrap">
                {tags.map((tag) => (
                  <li
                    key={tag}
                    className="font-semibold text-sm text-blue-600 bg-blue-200 inline rounded-full px-4 py-1 whitespace-nowrap"
                  >
                    {tag}
                  </li>
                ))}
