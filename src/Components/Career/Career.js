...
export default function Career() {
    ...
    return (
        <div className='flex flex-col items-center gap-y-16 mb-16'>
            <div className='relative max-w-fit w-[90%]'>
                <img src='/images/careers.jpeg' alt='' className='w-full aspect-video max-h-[87.5vh]' />
                <div className='absolute inset-0 bg-white bg-opacity-40 z-10'>
                    <div className='absolute left-10 top-1/2 -translate-y-1/2 space-y-8'>
                        <h2 className='font-bold text-2xl sm:text-3xl md:text-4xl'>Get Jobs</h2>
                        <div className='flex flex-wrap gap-x-4 gap-y-2'>
                            <input ref={title} placeholder='Job title' className='bg-white bg-opacity-30 text-lg sm:text-xl border border-black rounded-lg px-2 py-1 placeholder:text-black active:outline-none focus:outline-none' />
                            <select ref={location} placeholder='Location' className='bg-white bg-opacity-30 text-lg sm:text-xl border border-black rounded-lg px-2 py-1 active:outline-none focus:outline-none'>
                            ...
                            <div className='space-x-4'>
                                <button className='text-xl rounded-3xl cursor-pointer transition-all duration-500 px-5 py-1.5 font-mediu text-black bg-white bg-opacity-30 border border-black hover:scale-105 active:outline-none focus:outline-none' onClick={() => setFilter(title.current.value || location.current.value)}>Search</button>
                                {filter && <button className='text-xl rounded-3xl cursor-pointer transition-all duration-500 px-5 py-1.5 font-mediu text-black bg-white bg-opacity-30 border border-black hover:scale-105 active:outline-none focus:outline-none' onClick={reset}>Reset</button>}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className='space-y-8 px-8'>
                <div className='flex flex-wrap gap-x-12 gap-y-6 justify-between items-center'>
                    <h2 className='font-bold text-4xl'>{filter ? 'Filtered' : selectedTag} Jobs</h2>
